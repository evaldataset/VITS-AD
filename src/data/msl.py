...
def _load_msl_matrix(file_path: Path) -> FloatArray:
    """Load an MSL feature matrix from a .npy file.
    ...
    try:
        ...
        raise OSError(f"Failed to read MSL file: {file_path}") from exc
    except ValueError as exc:
        raise ValueError(f"Invalid MSL .npy file: {file_path}") from exc

    if matrix.ndim != 2:
        raise ValueError(
            f"MSL matrix must be 2D, got shape {matrix.shape} in {file_path}."
        )
    if matrix.shape[0] == 0 or matrix.shape[1] == 0:
        raise ValueError(f"MSL matrix must be non-empty: {file_path}")

    return np.asarray(matrix, dtype=np.float64)


def _load_msl_labels(file_path: Path) -> IntArray:
    """Load binary timestep labels for MSL from a .npy file.
    ...
    try:
        ...
        raise OSError(f"Failed to read MSL label file: {file_path}") from exc
    except ValueError as exc:
        raise ValueError(f"Invalid MSL label .npy file: {file_path}") from exc

    if labels.ndim != 1:
        raise ValueError(
            f"MSL labels must be 1D, got shape {labels.shape} in {file_path}."
        )
    if labels.shape[0] == 0:
        raise ValueError(f"MSL labels must be non-empty: {file_path}")

    return (labels > 0).astype(np.int64)


class MSLDataset:
    """MSL (Mars Science Laboratory) loader.

    Args:
        raw_dir: Path to raw MSL data directory.
    ...
    def __init__(
        ...
        if not raw_path.exists():
            raise ValueError(f"MSL raw_dir does not exist: {raw_path}")
        if not raw_path.is_dir():
            raise ValueError(f"MSL raw_dir must be a directory: {raw_path}")

        train_file = raw_path / "MSL_train.npy"
        test_file = raw_path / "MSL_test.npy"
        label_file = raw_path / "MSL_test_label.npy"

        for file_path in (train_file, test_file, label_file):
            if not file_path.exists():
                raise ValueError(f"Required MSL file does not exist: {file_path}")
            if not file_path.is_file():
                raise ValueError(f"Required MSL path is not a file: {file_path}")

        train_data = _load_msl_matrix(train_file)
        test_data = _load_msl_matrix(test_file)
        test_labels_timestep = _load_msl_labels(label_file)

        if train_data.ndim != 2 or test_data.ndim != 2:
            raise ValueError("MSL train/test data must be 2D arrays.")
        if train_data.shape[0] == 0 or test_data.shape[0] == 0:
            raise ValueError("MSL train/test data must be non-empty.")
        if train_data.shape[1] != test_data.shape[1]:
            raise ValueError(
                "MSL train and test feature dimensions must match, "
                f"got {train_data.shape[1]} and {test_data.shape[1]}."
            )
        if test_data.shape[0] != test_labels_timestep.shape[0]:
            raise ValueError(
                "MSL test data and test labels length mismatch, "
                f"got {test_data.shape[0]} and {test_labels_timestep.shape[0]}."
            )

        if normalize:
            logger.info("Normalizing MSL using method '%s'.", norm_method)
