#!/bin/bash
# Run LP+RP PatchTraj benchmark on the PSM dataset.
# Trains line_plot and recurrence_plot renderers, then ensembles via rank_mean.
#
# Usage:
#   bash scripts/run_psm_benchmark.sh GPU_ID
#
# Example:
#   bash scripts/run_psm_benchmark.sh 1
...
RESULTS_DIR="results/psm_benchmark"
mkdir -p "$RESULTS_DIR"

echo "=========================================="
echo "PSM LP+RP Benchmark on GPU $GPU_ID"
echo "=========================================="
...
for renderer in line_plot recurrence_plot; do
    ...
    echo ">>> Training PSM with $renderer..."
    ...
    echo ">>> Detecting PSM with $renderer..."
...
# Ensemble LP+RP
echo ""
echo ">>> Ensembling PSM (LP+RP)..."

# For PSM, ensemble_scores.py expects per-entity directories.
# We need to create the structure it expects.
# Since PSM has no entities, use a flat approach: manually combine scores.
python3 -c "
import json
import numpy as np
