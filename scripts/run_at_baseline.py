...
DATASET_CONFIGS: dict[str, ATConfig] = {
    "smd": ATConfig(
        data_name="SMD.csv",
        enc_in=38,
        model_hyper_params={
...
def _extract_scores_and_labels(
    ...
    from ts_benchmark.baselines.self_impl.Anomaly_trans.AnomalyTransformer import (  # type: ignore[import-not-found]
        AnomalyTransformer,
    )
    ...
    model = AnomalyTransformer(**config.model_hyper_params)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run AnomalyTransformer baseline and export VITS-compatible metrics."
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Directory to store AnomalyTransformer outputs and metrics.",
    )
...
def main() -> None:
    ...
    LOGGER.info(
        "Running AnomalyTransformer for data=%s on gpu=%d", config.data_name, args.gpu
    )
    ...
    result_path = output_dir / "at_results.json"
    with result_path.open("w", encoding="utf-8") as handle:
        json.dump(result_payload, handle, indent=2, sort_keys=True)

    LOGGER.info("Saved AnomalyTransformer baseline outputs to %s", output_dir)
    LOGGER.info(
